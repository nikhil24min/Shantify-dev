...
from .forms import UserRegisterForm,GUProfilefForm
from .models import GUprofile
...
def gulogout(request):
    logout(request)
    return redirect("gulogin")
...
def guregister(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            #login(request, user)
            GUprofile.objects.create(user = user)
            messages.success(request, "Registration successful.Plaese complete your profile" )
            print("success")
            return redirect("gulogin")
...
def guprofile(request):
    profile = request.user.guprofile
    print(profile)
    form = GUProfilefForm(instance=profile)
